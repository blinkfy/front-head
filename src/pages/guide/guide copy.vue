<template>
  <view class="guide-container">
    ...
    <view class="hero-section">
      <view class="video-card" @click="togglePlay">
        <video 
          id="promoVideo"
          ref="videoRef"
          class="promo-video"
        ...
        <view v-if="!isPlaying" class="video-overlay">
          ...
          <text class="overlay-title">分投侠宣传片</text>
          <text class="overlay-subtitle">点击探索智能分类新方式</text>
        </view>
        
        <!-- 静音切换按钮 -->
        <view v-if="isPlaying" class="mute-toggle" @tap.stop="toggleMute">
...
// 视频地址与封面
// 使用后端提供的 /files 接口访问 downloads 目录下的资源
const baseUrl = config.baseUrl
const videoUrl = computed(() => {
  return `${baseUrl}/files/download/%E5%88%86%E6%8A%95%E4%BE%A0%E8%AE%BE%E8%AE%A1/%E5%88%86%E6%8A%95%E4%BE%A0%E5%AE%A3%E4%BC%A0%E7%89%87/%E5%88%86%E6%8A%95%E4%BE%A0%E8%AE%BE%E8%AE%A1.m3u8`
})
const videoSource = computed(() => {
  if (isH5.value && hlsSupported.value) {
    return ''
  }
  return videoUrl.value
})
const videoPoster = `${baseUrl}/images/tip1.webp` // 暂时使用分类图标作为封面，建议后续替换为视频截图

function getVideoElement() {
  if (typeof window === 'undefined') return null
  if (videoRef.value && videoRef.value.tagName === 'VIDEO') {
    return videoRef.value
  }
  if (videoRef.value && videoRef.value.$el) {
    const host = videoRef.value.$el
    if (host && typeof host.querySelector === 'function') {
      return host.querySelector('video') || host
    }
    return host
  }
  return document.getElementById('promoVideo')
}

async function initHlsPlayer() {
  if (!isH5.value || hlsReady.value) return
  ...
  try {
    ...
    if (Hls && Hls.isSupported()) {
      ...
      hlsInstance.value.on(Hls.Events.ERROR, (_event, data) => {
        console.warn('[guide] HLS error:', data)
      })
      return
    }
  } catch (error) {
    console.warn('[guide] Failed to initialize hls.js, fallback to native video:', error)
  }
    ...
    if (hlsSupported.value) {
      ...
      if (isPlaying.value) {
        ...
        try {
          ...
          console.warn('[guide] HLS play failed:', error)
        }
      }
      return
    }
...
function onVideoError() {
  ...
  // #ifdef APP-PLUS
  const encoded = encodeURIComponent(videoUrl.value)
  uni.navigateTo({
    url: `/pages-nonTheme/webview?url=${encoded}`
  })
  // #endif
}
