export default function initPage() {
  ...
  const state = {
    ...
    lastMapSignature: '',
    routeStrategy: normalizeStrategy(getStorage('collection_route_strategy')),
  ...
  function saveKpiSeries(list) {
    try {
      setStorage(KPI_HISTORY_KEY, JSON.stringify(list))
    } catch (_) { }
  }
  ...
  async function refresh(options) {
    ...
    try {
      ...
      if (state.plan && state.plan.options && state.plan.options.routeStrategy) {
        ...
        setStorage('collection_route_strategy', state.routeStrategy)
        applyStrategyUI()
      }
  ...
  const init = (async function initFn() {
    applyTheme();
    // #ifdef H5
    ...
    // #endif
    tick();
    const tickTimer = addTimer(setInterval(tick, 1000));
    bind();
    applyStrategyUI();
    ...
    bindViewportResize();
    await refresh();
    state.timer = addTimer(setInterval(() => refresh({ silent: true }), 60000));
