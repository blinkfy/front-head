...
function apiRequest(path, options) {
  return new Promise((resolve, reject) => {
    ...
    fetch(url, { ...opts2, headers: { ...authHeaders(), ...(opts2.headers || {}) } })
      .then(r => r.json())
      .then(json => {
        if (!json || json.code !== 0) throw new Error((json && json.msg) || 'HTTP error')
        resolve(json.data)
      }).catch(reject)
    // #endif
    // #ifndef H5
...
async function doCreatePlan() {
  ...
  try {
    const payload = {
      ...
      options: { ...opts, startTime: new Date().toISOString() },
    ...
    if (plan.route && plan.route.stops && plan.route.stops.length) {
      const r = plan.route
      routeSummary.value = `Stops ${r.stops.length}, distance ${Number(r.totalDistanceKm || 0).toFixed(2)} km, total ${Number(r.totalMinutes || 0).toFixed(1)} min.`
    } else {
