...
const STRATEGY_STORAGE_KEY = 'collection_route_strategy'
...
const state = {
  ...
  infoWindow: null,
  startPoint: null,
  routeStrategy: normalizeStrategy(localStorage.getItem(STRATEGY_STORAGE_KEY))
}
...
const refs = {
  ...
  routeStrategy: document.getElementById('routeStrategy'),
...
async function loadTencentMapSdk() {
  ...
  for (const key of QQ_MAP_KEYS) {
    try {
      ...
      console.warn('[planning-map] sdk load failed:', error && error.message ? error.message : error)
    }
  }
...
function syncStrategyInput() {
  refs.routeStrategy.value = state.routeStrategy
}
...
function readStrategyFromInput() {
  state.routeStrategy = normalizeStrategy(refs.routeStrategy.value)
  localStorage.setItem(STRATEGY_STORAGE_KEY, state.routeStrategy)
  syncStrategyInput()
}
...
function renderBinList() {
  ...
  refs.binList.innerHTML = state.bins.map((bin, index) => {
    ...
    const hoursText = bin.hoursToFull == null ? '--' : `${Number(bin.hoursToFull).toFixed(1)}h`
    return `
      <div class="bin-row" data-index="${index}">
        <div class="bin-head">
          <label style="display:flex;align-items:center;gap:6px;">
            ...
            <span>${escapeHtml(bin.name || 'Unnamed Bin')}</span>
          </label>
          <span class="chip ${chipClass}">${chipText}</span>
        </div>
        <div class="bin-sub">
          满载率 ${Number(bin.currentFill || 0).toFixed(1)}% | 预计满载 ${hoursText} | 增长 ${Number(bin.growthRatePctPerHour || 0).toFixed(2)}%/h
        </div>
        <div class="bin-range">
...
function bindBinEvents() {
  refs.binList.addEventListener('input', (event) => {
    ...
    if (target.classList.contains('fill-range') || target.classList.contains('fill-input')) {
      updateBinFill(index, target.value)
    }
  })
  ...
  refs.binList.addEventListener('change', (event) => {
    ...
    if (target.classList.contains('row-check')) {
      ...
      state.bins[index].selected = !!target.checked
      drawMap()
    }
  })
...
function drawMap() {
  ...
  if (binGeometries.length) {
    ...
    state.binMarkers.on('click', (event) => {
      ...
      showInfoWindow(
        bin.latitude,
        bin.longitude,
        `<div style="font-size:12px;line-height:1.5;"><b>${escapeHtml(bin.name || 'Unnamed Bin')}</b><br/>满载率 ${Number(bin.currentFill || 0).toFixed(1)}%<br/>预计满载 ${bin.hoursToFull == null ? '--' : `${Number(bin.hoursToFull).toFixed(1)}h`}<br/>优先级 ${Number(bin.priorityScore || 0).toFixed(3)}</div>`
      )
    })
  }
  ...
  if (
    ...
    const stopStyles = {}
    const stopGeometries = (state.plan.route.stops || []).map((stop, index) => {
...
function renderRoute() {
  if (!state.plan || !state.plan.route || !Array.isArray(state.plan.route.stops) || !state.plan.route.stops.length) {
    refs.routeSummary.textContent = '还未生成路线。'
  ...
  refs.routeSummary.textContent =
    `${strategyLabel(state.routeStrategy)} | 停靠 ${route.stops.length} 个，里程 ${Number(route.totalDistanceKm || 0).toFixed(2)} km，总耗时 ${Number(route.totalMinutes || 0).toFixed(1)} min。`

  refs.routeList.innerHTML = route.stops.map((stop) => `
    <div class="route-item">
      <div class="route-order">${stop.order}</div>
      <div>
        <div style="font-weight:680;">${escapeHtml(stop.name)}</div>
      ...
      <div style="text-align:right;color:#445d73;">
        ...
        <div style="font-size:11px;">${Number(stop.travelMinutes).toFixed(1)} min</div>
      </div>
    </div>
  `).join('')
}
...
async function loadBins() {
  ...
  updateMetrics()
  drawMap()
  setStatus(`已加载 ${state.bins.length} 个桶位。`, 'ok')
}
...
async function createPlan() {
  ...
  setStatus(`正在计算${strategyLabel(state.routeStrategy)}路线...`, 'warn')
  ...
  if (plan && plan.options && plan.options.routeStrategy) {
    state.routeStrategy = normalizeStrategy(plan.options.routeStrategy)
    localStorage.setItem(STRATEGY_STORAGE_KEY, state.routeStrategy)
    syncStrategyInput()
  }
...
async function saveSnapshots() {
  ...
  if (!targets.length) {
    setStatus('请先至少选择一个桶位。', 'err')
    return
  }

  setStatus('正在保存当前快照...', 'warn')
  ...
  setStatus(`已保存 ${targets.length} 个桶位快照。`, 'ok')
}

async function createMockHistory() {
  const targets = state.bins
    .filter((bin) => bin.selected !== false)
    .map((bin) => bin.id)
  if (!targets.length) {
...
function bindActions() {
  ...
  refs.routeStrategy.addEventListener('change', () => {
    readStrategyFromInput()
    renderRoute()
  })
}
...
;(async function init() {
  ...
  window.addEventListener('storage', (event) => {
    ...
    if (!event || event.key === STRATEGY_STORAGE_KEY) {
      state.routeStrategy = normalizeStrategy(localStorage.getItem(STRATEGY_STORAGE_KEY))
      syncStrategyInput()
    }
  })
