  ...
  <view class="inspector panel">
    ...
    <view v-if="selectedId" class="entity-summary">
      ...
      <view class="entity-heading">
        <view class="entity-name">{{ labels[selectedId] || selectedId }}</view>
...
const entry = computed(() => props.idMap?.entries?.[props.selectedId] || {})
...
const glyph = computed(() => ({ robot: 'R', service_point: 'P', device: 'B', sorting_center: 'C', task: 'T', garbage: 'W', visitor: 'V' })[entry.value.type] || 'O')
const presentation = computed(() => eventPresentation(props.currentEvent))
const explicitStatus = computed(() => explicitEntityStatus(props.currentEvent, props.selectedId))
const statusTone = computed(() => /RISK|RETURN|PENDING|CHARGING/i.test(explicitStatus.value) ? 'warning' : /SUCCEEDED|ONLINE|OCCUPIED|STANDBY/i.test(explicitStatus.value) ? 'success' : '')
...
const finalRecord = computed(() => {
  ...
  return state.devices?.[props.selectedId] || state.servicePoints?.[props.selectedId] ||
    state.tasks?.[props.selectedId] || (state.robot?.id === props.selectedId ? state.robot : null) ||
    (state.sortingCenter?.id === props.selectedId ? state.sortingCenter : null) || null
})
const finalStatus = computed(() => finalRecord.value?.status || '无状态字段')
...
const relatedTasks = computed(() => {
  ...
  return Object.entries(tasks)
    ...
    .map(([id, task]) => ({ id, status: task.status || '—' }))
})
</script>
