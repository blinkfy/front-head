<template>
  <view class="decision-panel panel">
    ...
    <view v-if="!trace" class="decision-empty">
      <text class="b-text">该事件无调度决策</text>
      <text>仅在实际发生派单、预留、泊位分配、路径重规划或异常响应时显示。</text>
    </view>

    <template v-else>
      <view class="decision-meta">
        <view><text>决策编号</text><text class="b-text">{{ trace.decisionId || '—' }}</text></view>
        <view v-if="trace.taskId"><text>关联任务</text><text class="b-text">{{ trace.taskId }}</text></view>
        <view v-if="payload.reservationId"><text>预留编号</text><text class="b-text">{{ payload.reservationId }}</text></view>
        <view v-if="trace.reassignedFrom || payload.reassignedFrom"><text>重新分配来源</text><text class="b-text">{{ trace.reassignedFrom || payload.reassignedFrom }}</text></view>
        <view v-if="isCenterDecision"><text>中心等待队列</text><text class="b-text">{{ centerQueueText }}</text></view>
        <view v-if="isIncidentDecision"><text>当前恢复阶段</text><text class="b-text">{{ payload.currentStage || '—' }}</text></view>
        <view v-if="isRiskDecision"><text>当前填充率</text><text class="b-text">{{ predictionResult?.currentFillPct ?? '—' }}%</text></view>
        <view v-if="isRiskDecision"><text>当前增长率</text><text class="b-text">{{ predictionResult?.growthRatePctPerHour ?? '—' }}%/h</text></view>
        <view v-if="isRiskDecision"><text>预计满载</text><text class="b-text">{{ fullTimeText }}</text></view>
        <view v-if="isRiskDecision"><text>风险等级</text><text class="b-text">{{ riskLevelText }}</text></view>
      </view>

      <view v-if="scoreRows.length" class="score-summary">
        <view v-for="item in scoreRows" :key="item.key"><text>{{ item.label }}</text><text class="b-text">{{ item.value }}</text></view>
      </view>

      <view class="candidate-section">
        <view class="section-title"><text>候选对象</text><text class="b-text">{{ candidateRows.length }}</text></view>
        ...
        >
          <view class="candidate-top">
            <view class="candidate-name">
              <text class="b-text">{{ candidateLabel(candidate.entityId) }}</text>
              <text class="small-text">{{ candidate.entityId }}</text>
            </view>
          ...
          <view v-if="candidateCost(candidate) != null" class="cost-row">
            ...
            <text class="b-text">{{ scoreCaption }} {{ candidateCost(candidate) }}</text>
          </view>
      ...
      <view class="selected-reason">
        ...
        <text class="b-text">{{ trace.selectedReason || '未记录额外说明' }}</text>
      </view>
    </template>
  </view>
</template>
...
.decision-empty { padding:18px 12px;text-align:center }.decision-empty .b-text,.decision-empty text { display:block }.decision-empty .b-text { color:#91adba;font-size:10px }.decision-empty text { margin-top:5px;color:#5f8293;font-size:7px;line-height:1.5 }
.decision-meta { display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;padding:9px 12px;border-bottom:1px solid rgba(116,197,255,.1) }.decision-meta view,.score-summary view { min-width:0;padding:5px 6px;border-radius:5px;background:rgba(27,81,111,.18) }.decision-meta text,.decision-meta .b-text,.score-summary text,.score-summary .b-text { display:block }.decision-meta text,.score-summary text { color:#7299ac;font-size:7px }.decision-meta .b-text,.score-summary .b-text { margin-top:2px;color:#d8eef7;font:7px/1.3 ui-monospace,Consolas,monospace;overflow-wrap:anywhere }
.score-summary { display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:4px;padding:7px 12px;border-bottom:1px solid rgba(116,197,255,.1) }.score-summary .b-text { color:#ffd27d }
.candidate-section { padding:9px 12px }.section-title { display:flex;justify-content:space-between;color:#8bc9e7;font-size:9px;font-weight:700;margin-bottom:6px }.section-title .b-text { color:#5f91aa }.candidate-row { padding:7px;margin-top:5px;border:1px solid rgba(116,197,255,.13);border-radius:6px;background:rgba(8,42,62,.34) }.candidate-row.selected { border-color:rgba(44,220,166,.52);background:linear-gradient(90deg,rgba(26,159,116,.15),rgba(8,42,62,.34)) }.candidate-row.excluded { opacity:.72 }.candidate-top { display:flex;align-items:center;gap:6px }.candidate-name { min-width:0;flex:1 }.candidate-name .b-text,.candidate-name .small-text { display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap }.candidate-name .b-text { color:#dff4fb;font-size:8px }.candidate-name .small-text { margin-top:2px;color:#5f91aa;font:6px/1.2 ui-monospace,Consolas,monospace }.candidate-state { color:#8ab0c1;font-size:6px }.candidate-state.selected { color:#73edb5 }.candidate-state.excluded { color:#ffb071 }.candidate-facts { display:flex;flex-wrap:wrap;gap:4px;margin-top:5px }.candidate-facts text { padding:2px 4px;border-radius:3px;color:#9ec4d4;background:rgba(80,145,174,.12);font-size:6px }.cost-row { display:flex;align-items:center;gap:6px;margin-top:6px }.cost-track { height:3px;flex:1;border-radius:9px;overflow:hidden;background:rgba(103,157,181,.16) }.cost-fill { height:100%;border-radius:inherit;background:linear-gradient(90deg,#24d9ff,#5d91ff) }.cost-row .b-text { color:#9dc9da;font-size:6px }.excluded-reason { margin-top:5px;padding-left:6px;border-left:2px solid #d9884e;color:#e5ad82;font-size:7px;line-height:1.35 }.candidate-empty { padding:10px;text-align:center;color:#6f93a6;font-size:8px }
.selected-reason { padding:9px 12px;border-top:1px solid rgba(116,197,255,.12);background:linear-gradient(90deg,rgba(36,217,255,.05),transparent) }.selected-reason text,.selected-reason .b-text { display:block }.selected-reason text { color:#71a0b4;font-size:7px }.selected-reason .b-text { margin-top:4px;color:#dff5fc;font-size:8px;line-height:1.45 }
.panel-title { font-size:15px }.panel-sub { font-size:10px }.decision-type { font-size:10px }
.decision-empty .b-text { font-size:12px }.decision-empty text { font-size:9px }
.decision-meta text,.score-summary text { font-size:9px }.decision-meta .b-text,.score-summary .b-text { font-size:9px }
.section-title { font-size:11px }.candidate-name .b-text { font-size:10px }.candidate-name .small-text { font-size:8px }
.candidate-state,.candidate-facts text,.cost-row .b-text { font-size:8px }.excluded-reason { font-size:9px }
.candidate-empty { font-size:10px }.selected-reason text { font-size:9px }.selected-reason .b-text { font-size:10px }

/* #ifdef MP-WEIXIN */
.decision-meta { display: flex; flex-wrap: wrap; }
.decision-meta view { flex: 1; min-width: 120px; }
.score-summary { display: flex; flex-wrap: wrap; }
.score-summary view { flex: 1; min-width: 80px; }
/* #endif */
</style>
