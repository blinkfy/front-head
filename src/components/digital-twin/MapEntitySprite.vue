<template>
  <view :class="['map-entity-sprite', kind, variant, direction.key, { selected, affected, active, moving }]" :style="spriteStyle" :data-asset-status="resolvedAssetStatus" :data-asset-source="sprite.source" :data-heading-direction="direction.key">
    ...
    <view v-if="kind === 'robot'" class="entity-heading-indicator" aria-hidden="true"><i class="heading-arrow"></i></view>
    <view class="entity-art">
      <image v-if="sprite.src" :class="`${kind}-image`" :src="sprite.src" mode="aspectFit" />
      <view v-else-if="kind === 'center'" class="center-placeholder"><i class="center-placeholder-door"></i><text class="b-text"></text><text class="em-label"></text></view>
      <view v-else-if="kind === 'station'" class="station-placeholder"><i class="station-placeholder-line"></i><text class="b-text"></text></view>
    </view>
...
.heading-arrow { position:absolute; right:-1px; top:-2px; width:0; height:0; border-top:3px solid transparent; border-bottom:3px solid transparent; border-left:4px solid rgba(168,222,232,.88); }
...
.map-entity-sprite.center { width:160px; height:106px; place-items:center; }.map-entity-sprite.center::before { content:''; position:absolute; z-index:0; inset:13px 4px 7px; border:1px solid rgba(62,205,242,.24); border-radius:10px; background:linear-gradient(145deg,rgba(8,57,76,.2),rgba(3,25,39,.5)); box-shadow:inset 0 0 24px rgba(36,217,255,.08),0 8px 18px rgba(0,12,20,.28); transform:skewY(-2deg); }.center-image { position:relative; z-index:1; width:154px; height:102px; object-fit:contain; }.map-entity-sprite.center.active::before { border-color:rgba(36,217,255,.62); box-shadow:inset 0 0 28px rgba(36,217,255,.14),0 0 18px rgba(36,217,255,.2); }.map-entity-sprite.center .entity-ground-shadow { width:112%; height:18%; bottom:3px; transform:translate(calc(-50% + 16px),8px) rotate(14deg) skewX(-6deg); clip-path:none; border-radius:52% 42% 58% 44%; background:radial-gradient(ellipse at 24% 44%,rgba(0,12,20,.4) 0 13%,rgba(0,12,20,.22) 35%,rgba(0,12,20,.08) 60%,transparent 82%),radial-gradient(ellipse at 62% 55%,rgba(0,12,20,.12) 0 22%,transparent 68%); filter:blur(8px); opacity:.72; }.map-entity-sprite.center .entity-contact-shadow { width:54%; height:5%; bottom:8px; transform:translate(calc(-50% + 2px),0); clip-path:none; background:radial-gradient(ellipse at 50% 50%,rgba(0,10,17,.14) 0 28%,rgba(0,10,17,.05) 62%,transparent 82%); filter:blur(2.5px); opacity:.18; }.center-placeholder { position:relative; width:48px; height:27px; border:1px solid #9bd7ec; border-radius:3px; background:linear-gradient(145deg,#2e6d88,#13384b); transform:skewY(-4deg); }.center-placeholder::before { content:''; position:absolute; left:3px; right:3px; top:-9px; height:11px; border:1px solid #bceeff; background:linear-gradient(155deg,#6aa6bc,#285c73); transform:skewY(4deg); }.center-placeholder .center-placeholder-door,.center-placeholder .b-text,.center-placeholder .em-label { position:absolute; bottom:3px; width:8px; height:13px; border:1px solid rgba(210,244,255,.7); background:#092131; }.center-placeholder .center-placeholder-door{left:5px}.center-placeholder .b-text{left:19px}.center-placeholder .em-label{right:5px}.center-placeholder.active{box-shadow:inset 0 0 12px rgba(36,217,255,.35)}
.map-entity-sprite.station { width:34px; height:31px; }.station-image { width:34px; height:31px; object-fit:contain; }.map-entity-sprite.station.active .station-image { filter:drop-shadow(0 0 2px #fff) drop-shadow(0 0 7px #24d9ff); }.station-placeholder { position:relative; width:20px; height:18px; border:1px solid #9bd7ec; border-radius:3px; background:linear-gradient(145deg,#376f83,#163849); transform:skewY(-5deg); }.station-placeholder-line { position:absolute; left:3px; right:3px; top:4px; height:2px; background:#24d9ff; }.station-placeholder .b-text { position:absolute; left:7px; bottom:2px; width:6px; height:7px; border:1px solid rgba(220,249,255,.75); }.map-entity-sprite.station.active .station-placeholder { border-color:#fff; box-shadow:0 0 10px #24d9ff; }
